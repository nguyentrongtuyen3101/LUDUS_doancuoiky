...
export class createDto {
    ...
    validate() {
        ...
        if (isNaN(this.price) || this.price <= 0) throw new ClientException("Giá sản phẩm phải là số dương", 400);
        if (isNaN(this.stockQuantity) || this.stockQuantity < 0) throw new ClientException("Số lượng tồn kho phải là số không âm", 400);
        if (this.imageUrl && !/\.(jpg|jpeg|png|gif|bmp|webp)$/i.test(this.imageUrl)) {throw new ClientException("Định dạng hình ảnh không hợp lệ", 400);}
